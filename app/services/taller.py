            ...
            try:
                ...
                if fab:
                    ...
                    if (prod.stock_reservado or 0) >= consumo - 1e-9:
                        ...
                        prod.stock_fisico = round(max(0, (prod.stock_fisico or 0) - consumo), 2)
                    # Kardex SALIDA_CONSUMO_TALLER valorizado a CPP + asiento
                    # de consumo a WIP (regla por categoría) en la transacción.
...
def aprobar_calidad(db: Session, gid: int, checks: list[bool], observaciones: str,
    ...
    g.paso_confeccion = True
    db.flush()
    # ENTRADA física obligatoria en Almacén Cta 23 (Productos Terminados):
    # la prenda LISTA PARA ENTREGA (CALIDAD_OK) se registra en Kardex como
    # ENTRADA_PRODUCTO_TERMINADO. Sin insumo asociado (producto_id NULL) y
    # sin asiento: la valorización vive en el flujo de costo (bespoke a la
    # entrega/factura 69 vs 23, colección al cierre de OP). Idempotente por
    # prenda: si ya existe la entrada, no se duplica.
    try:
        from app.models.inventario import MovimientoKardex
        order = db.get(Order, g.order_id)
        folio = order.folio if order and order.folio else f"OV#{g.order_id}"
        marca = f"PT {g.tipo} #{g.id} lista ({folio})"
        ya = db.query(MovimientoKardex).filter(
            MovimientoKardex.orden_venta_id == g.order_id,
            MovimientoKardex.tipo_movimiento == "ENTRADA_PRODUCTO_TERMINADO",
            MovimientoKardex.observacion == marca).first()
        if not ya:
            db.add(MovimientoKardex(
                producto_id=None, tipo_movimiento="ENTRADA_PRODUCTO_TERMINADO",
                cantidad=1, costo_unitario=0.0, costo_total=0.0,
                saldo_fisico=0.0, saldo_valorizado=0.0,
                orden_venta_id=g.order_id, asiento_id=None, doc_ref=folio,
                usuario_id=usuario_id, observacion=marca))
            db.flush()
    except Exception:
        pass
    db.commit()
