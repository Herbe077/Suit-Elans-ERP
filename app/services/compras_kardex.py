...
# Reglas de consumo por categoría (Plan Operativo, sin cuentas cableadas).
# TELA_PRINCIPAL y FORROS (más aliases legacy) → CONSUMO_MPD (2311x/2411x);
# entretelas, avíos y empaques → CONSUMO_AUX (2311x/2521x).
CATEGORIAS_TELA = {"TELA_PRINCIPAL", "FORROS", "TELA", "TELAS",
                   "FORRO", "FORROS"}


def regla_consumo_por_categoria(categoria: str | None) -> str:
    """Código de regla de consumo según categoría del insumo."""
    from app.models.inventario import es_tela
    if es_tela(categoria):
        return "CONSUMO_MPD"
    return "CONSUMO_AUX"


def dims_consumo(db, prod: ProductoInsumo, **extra) -> dict:
    """Dims base de consumo: producto + centro taller + almacén principal."""
    from app.services.motor_contable import dim_almacen, dim_centro
    dims = {"producto_id": prod.id,
            "centro_costo_id": dim_centro(db, "921"),
            "almacen_id": dim_almacen(db)}
    dims.update({k: v for k, v in extra.items() if v is not None})
    return dims


def asiento_consumo_flush(db, prod: ProductoInsumo,
                          cantidad: float | Decimal,
                          cpp: float | Decimal | None,
                          glosa: str, fecha,
                          usuario_id: int | None = None,
                          **dims_extra):
    """Asiento de consumo a WIP por regla (flush; None si valorizado <= 0).

    El Kardex físico igual se registra sin asiento (sin efecto monetario).
    """
    from app.services.motor_contable import post_regla
    valorizado = _d(cantidad) * _d(cpp)
    if valorizado <= 0:
        return None
    return post_regla(
        db, regla_consumo_por_categoria(getattr(prod, "categoria", None)),
        [valorizado], [valorizado], dims_consumo(db, prod, **dims_extra),
        glosa, "INVENTARIO", prod.id, fecha)
...
def recepcionar_oc(db: Session, oc_id: int, recepciones: dict[int, float],
    ...
    try:
        ...
        for d, qty in items:
            ...
            k = MovimientoKardex(
                producto_id=prod.id, tipo_movimiento="ENTRADA_COMPRA",
        ...
        # Un asiento RECEPCION_MP por línea (dims producto + almacén).
        from app.services.motor_contable import dim_almacen
        alm_id = dim_almacen(db)
        asientos = []
        for k in kardex_rows:
            a = post_regla(
                db, "RECEPCION_MP", [k.costo_total], [k.costo_total],
                {"producto_id": k.producto_id, "almacen_id": alm_id},
                f"Recepción OC {oc.folio or oc.id}", "INVENTARIO", oc.id, fecha)
            k.asiento_id = a.id
            asientos.append(a)
        asiento = asientos[0]
        db.flush()
        ...
        # CxP forma parte del mismo evento económico: si falla, se revierte
        # recepción + kardex + asiento como una sola unidad ACID.
...
def facturar_oc(db: Session, oc_id: int, numero_factura: str,
    ...
    try:
        ...
        from app.services.motor_contable import post_regla
        asiento = post_regla(
            db, "COMPRA_MP", [base, igv], [base + igv],
            {"proveedor_id": oc.proveedor_id},
            f"Provisión {numero_factura} OC {oc.folio or oc.id}",
            "COMPRA", oc.id, fecha)
...
def _asiento_provision_oc(db: Session, oc) -> object | None:
    """Backfill de la provisión COMPRA_MP para una OC BILLED sin provisión.
    ...
    try:
        ...
        return post_regla(
            db, "COMPRA_MP", [base, igv], [total],
            {"proveedor_id": oc.proveedor_id},
            f"Provisión {(oc.numero_factura or '').strip() or oc.folio or oc.id} OC {oc.folio or oc.id}",
            "COMPRA", oc.id, oc.fecha_factura or date.today())
    except Exception:
        return None


def reparar_cxp_compras(db: Session, commit: bool = True) -> dict:
...
# ── Consumo a WIP y merma (reglas CONSUMO_* / MERMA) ────────────────
def consumir_taller(db: Session, producto_id: int, cantidad: float,
    ...
    """Salida a producción valorizada a CPP (regla por categoría → SALIDA_CONSUMO_TALLER)."""
    ...
    try:
        ...
        asiento = asiento_consumo_flush(
            db, prod, qty, cpp,
            f"Consumo taller {prod.sku} x{qty} {doc_ref or ''}".strip(),
            fecha, usuario_id)
        k = MovimientoKardex(
            producto_id=prod.id, tipo_movimiento="SALIDA_CONSUMO_TALLER", cantidad=float(qty),
            ...
            asiento_id=asiento.id if asiento else None, doc_ref=doc_ref,
...
def registrar_merma(db: Session, producto_id: int, cantidad: float,
                    ...
                    fecha: date | None = None) -> dict:
    """Ajuste por merma/desmedro valorizado a CPP (regla MERMA)."""
    from app.services.motor_contable import post_regla
    if not cantidad or _d(cantidad) <= 0:
        raise ValueError("Cantidad debe ser positiva")
    fecha = fecha or date.today()
    ...
    try:
        ...
        if valorizado > 0:
            asiento = post_regla(
                ...
                f"Merma {prod.sku} x{qty}", "INVENTARIO", prod.id, fecha)
        k = MovimientoKardex(
            producto_id=prod.id, tipo_movimiento="SALIDA_MERMA", cantidad=float(qty),
            costo_unitario=float(cpp), costo_total=float(valorizado),
            saldo_fisico=float(prod.stock_fisico),
            saldo_valorizado=float(_d(prod.stock_fisico) * cpp),
            asiento_id=asiento.id if asiento else None,
