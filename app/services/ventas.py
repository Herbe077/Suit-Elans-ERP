...
METODOS_MAP = {"efectivo": "EFECTIVO", "yape": "YAPE", "plin": "PLIN",
...
def registrar_cobro(db: Session, order_id: int, monto: float, metodo: str,
    ...
    metodo_norm = METODOS_MAP.get((metodo or "").lower(), (metodo or "EFECTIVO").upper())
    if metodo_norm not in METODOS_PAGO:
    ...
    res = contab.cobrar_venta(db, order_id, float(monto), metodo_norm.lower(),
