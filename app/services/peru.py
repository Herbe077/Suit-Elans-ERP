...
# Tope SUNAT: boletas a Público General (S/D) solo por montos MENORES.
SUNAT_BOLETA_PG_TOPE = 700.0
MENSAJE_SUNAT_PG = ("⚠️ Según normativa SUNAT, para ventas desde S/ 700.00 "
                    "es obligatorio registrar los datos del cliente "
                    "(DNI/CE y Nombres).")


def exigir_cliente_sunat(total: float | None, tiene_cliente: bool) -> None:
    """Bloquea boleta a Público General desde el tope SUNAT.

    Lanza ValueError con el mensaje normativo si el total es igual o mayor
    a S/ 700.00 y no hay cliente/empresa identificado.
    """
    if round(float(total or 0), 2) >= SUNAT_BOLETA_PG_TOPE and not tiene_cliente:
        raise ValueError(MENSAJE_SUNAT_PG)
