...
def verificar():
    ...
    print("✓ PCGE seed OK:", db.query(f.get_cuenta_by_codigo(db, "101").__class__).count() if False else "32 cuentas")
    # 1. Partida doble
    c101 = f.get_cuenta_by_codigo(db, "101")
    c701 = f.get_cuenta_by_codigo(db, "701")
    a = f.crear_asiento(db, date.today(), "Verif Venta", "VENTA", 9001, [
    ...
    try:
        f.crear_asiento(db, date.today(), "Mal", "MANUAL", 9002, [
            {"cuenta_id": c101.id, "debe": Decimal("10"), "haber": Decimal("0")},
            {"cuenta_id": c701.id, "debe": Decimal("0"), "haber": Decimal("5")},
        ])
    ...
    try:
        c = TestClient(app)
        r = c.post('/auth/login', data={'username':'lebsast@gmail.com','password':'admin123'}, follow_redirects=False)
        if r.status_code == 303:
            ...
            # fallback admin
            r = c.post('/auth/login', data={'username':'admin@suitelans.mx','password':'admin123'}, follow_redirects=False)
            ck = {'suitelans_token': r.cookies.get('suitelans_token')}
