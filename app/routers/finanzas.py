...
from app.services.plan_operativo import asegurar_plan_operativo as _plan_loader


def _asegurar_plan(db):
    """Bootstrap del plan operativo (solo si no hay cuentas)."""
    r = _plan_loader(db)
    if r:
        db.commit()
...
def _sync_cxp(db):
    ...
    for po in db.query(PurchaseOrder).filter(PurchaseOrder.estado.notin_(["cancelada", "CANCELADA", "recibida", "RECIBIDA", "COMPLETADA"])).all():
        if not db.query(CuentaPorPagar).filter(CuentaPorPagar.purchase_order_id==po.id).first():
            ...
            db.add(cxp)
            db.flush()
            # Provisión contable de la obligación por regla COMPRA_MP
            # (602x / 4212; sin IGV en este espejo legacy).
            ...
            try:
                ...
                if not ya and total > 0:
                    post_regla(
                        db, "COMPRA_MP", [total, 0], [total],
                        {"proveedor_id": cxp.proveedor_id},
                        f"Provisión CxP PO {po.folio or po.id}",
                        "COMPRA", cxp.id, date.today())
                    db.flush()
            except Exception as e:
...
def plan_crear(codigo: str=Form(...), nombre: str=Form(...), tipo: str=Form(...), nivel: int=Form(1), padre: str=Form(""),
               imputable: bool=Form(True), analitica: bool=Form(False),
               db: Session = Depends(get_db), user=FinanzasAuth):
    from app.models.finanzas import CuentaContable
    try:
        if db.query(CuentaContable).filter(CuentaContable.codigo == codigo.strip()).first():
            return HTMLResponse("Código duplicado", status_code=400)
        if tipo not in ("ACTIVO", "PASIVO", "PATRIMONIO", "GASTO", "INGRESO"):
            return HTMLResponse("Tipo inválido", status_code=400)
        db.add(CuentaContable(codigo=codigo.strip(), nombre=nombre.strip(), tipo=tipo,
                              nivel=nivel, padre_codigo=padre.strip() or None,
                              imputable=bool(imputable), analitica=bool(analitica),
                              acepta_movimientos=True, activo=True))
        db.commit()
    except Exception as e:
        db.rollback()
...
def apertura_crear(fecha: str = Form(""), caja: float = Form(0), banco: float = Form(0),
                   ...
                   db: Session = Depends(get_db), user=FinanzasAuth):
    # Asiento inicial: DEBE 1011+1041+2411+3351 / HABER 4212+5011.
    ...
    try:
        ...
        debe = [(c, v) for c, v in debitos.items() if v > 0]
        haber = (([("4212", pasivo)] if pasivo > 0 else [])
                 + [("5011", round(capital, 2))])
        post_manual(
            db, debe, haber, None, "Asiento de apertura — saldos iniciales",
            "APERTURA", None, fe)
        db.commit()
...
def diario(request: Request, periodo: str=Query(""), origen: str=Query(""), cuenta: str=Query(""),
    ...
    cuentas=db.query(CuentaContable).filter(CuentaContable.imputable==True).order_by(CuentaContable.codigo).all()
...
def cxp(request: Request, estado: str = Query(""), proveedor: str = Query(""), origen: str = Query(""), db: Session = Depends(get_db), user=FinanzasAuth):
    """Bandeja de solo lectura. Las nivelaciones se ejecutan fuera del GET.

    Esto evita que una consulta de tesorería mutile la contabilidad o bloquee
    el pool de conexiones con DDL/sincronizaciones masivas.
    """
    try:
        q = db.query(CuentaPorPagar, Supplier).outerjoin(
            Supplier, Supplier.id == CuentaPorPagar.proveedor_id)
        if estado:
            q = q.filter(CuentaPorPagar.estado == estado.upper())
        if origen:
            norm = svc.normalizar_origen_cxp(origen)
        ...
        if proveedor:
            try:
                ...
                q = q.filter(Supplier.nombre.ilike(f"%{proveedor}%"))
        rows = q.order_by(CuentaPorPagar.id.desc()).limit(200).all()
        ...
        if refs:
            movs = db.query(MovimientoFinanciero).filter(
                ...
                MovimientoFinanciero.comprobante_ref.in_(refs)
            ).order_by(MovimientoFinanciero.id.desc()).all()
            for m in movs:
        ...
        suppliers = _visibles(db)
        proveedores = {s.id: s for s in suppliers}
        return templates.TemplateResponse(request, "finanzas/cuentas_por_pagar.html", {
            "user": user, "tab": "cxp", "cuentas": cuentas,
            "proveedores": proveedores, "nombres_cxp": nombres_cxp,
            "medios_cxp": medios_cxp, "estado": estado, "proveedor": proveedor,
            "origen": origen, "origenes": list(svc.ORIGENES_CXP), "suppliers": suppliers,
        })
    except Exception as e:
        db.rollback()
...
def cxp_pagar(cid: int, monto: float = Form(...), cuenta_origen: str = Form("Banco"),
              permitir_sobregiro: str = Form(""), db: Session = Depends(get_db), user=FinanzasAuth):
    # Pago atómico: CxP + EGRESO flujo por naturaleza + caja + asiento al
    # pasivo real (4111 planilla / 4212 proveedores / 4241 RxH…; bloquea período cerrado).
    # 1011 bloquea sin saldo; 1041 exige el flag permitir_sobregiro.
    from app.services import contabilidad as contab
    try:
        contab.pagar_proveedor(db, cid, float(monto),
                               cuenta_codigo="1041" if cuenta_origen == "Banco" else "1011",
                               usuario_id=user.id,
                               permitir_sobregiro=(permitir_sobregiro or "").strip().lower() in ("1", "true", "on", "si"))
    except ValueError as e:
        return _error_pago_400("/finanzas/cuentas-por-pagar", str(e))
    return RedirectResponse("/finanzas/cuentas-por-pagar", status_code=303)


def _error_pago_400(back: str, msg: str) -> HTMLResponse:
    """Error 400 legible para pagos bloqueados (sobregiro, saldos, período).

    Mantiene el status 400 para clientes programáticos y muestra el motivo
    con enlace de retorno para el operador del modal de pago.
    """
    import html as _html
    safe_back = back if back.startswith("/") else "/finanzas/cuentas-por-pagar"
    cuerpo = (
        "<!DOCTYPE html><html lang='es'><head><meta charset='UTF-8'>"
        "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
        "<title>Pago bloqueado — Suit Elans</title></head>"
        "<body style='font-family:system-ui;background:#FAF8F5;color:#1F1112;"
        "display:flex;justify-content:center;padding:48px 16px'>"
        "<div style='background:#fff;border-top:3px solid #C5A880;border-radius:12px;"
        "box-shadow:0 2px 12px rgba(0,0,0,.08);padding:24px;max-width:480px;width:100%'>"
        "<h1 style='font-size:18px;margin:0 0 8px'>⛔ Pago bloqueado</h1>"
        f"<p style='font-size:14px;color:#444'>{_html.escape(str(msg))}</p>"
        f"<a href='{_html.escape(safe_back)}' style='display:inline-block;margin-top:12px;"
        "background:#370003;color:#fff;border-radius:8px;padding:10px 16px;"
        "text-decoration:none;font-weight:bold;font-size:14px'>← Volver</a>"
        "</div></body></html>")
...
def tesoreria_pagar(
    ...
    sobregiro_ok = (permitir_sobregiro or "").strip().lower() in ("1", "true", "on", "si")
    try:
        if (gasto_id or "").strip():
            tes.ejecutar_pago_proveedor(
                ...
                permitir_sobregiro=sobregiro_ok)
            ...
            return _error_pago_400(back, "Indica gasto_id o cxp_id")
    except ValueError as e:
        return _error_pago_400(back, str(e))
...
def gastos_list(request: Request, estado: str = Query(""), categoria: str = Query(""), error: str = Query(""), db: Session = Depends(get_db), user=FinanzasAuth):
    try:
        ...
        return templates.TemplateResponse(request, "finanzas/gastos.html", {
            ...
            "categorias": ["ALQUILERES", "HONORARIOS_TERCEROS",
                           "PLANILLA_PERSONAL", "SERVICIOS_BASICOS",
                           "COMPRAS_AVIOS_SUMINISTROS", "PUBLICIDAD_MARKETING",
                           "SUMINISTROS_Y_HERRAMIENTAS_TALLER",
                           "ACTIVO_FIJO", "OTROS_GASTOS"],
            "map": svc.CATEGORIA_GASTO_MAP,
            "retencion_4ta_pct": config_svc.parametros_laborales(db)["retencion_4ta_pct"],
        })
    except Exception as e:
        try:
            db.rollback()
...
def gastos_crear(
    ...
    centro_costo_id: str = Form(""), fecha_vencimiento: str = Form(""),
    actividad_flujo: str = Form(""), condicion_pago: str = Form("CREDITO"),
    ...
    try:
        fv = date.fromisoformat(fecha_vencimiento) if fecha_vencimiento else None
    except ValueError:
        return HTMLResponse("Fecha de vencimiento inválida", status_code=400)
    af = (actividad_flujo or "").upper() or None
    ...
    try:
        gasto, _as = contab.registrar_gasto_atomico(
            ...
            fecha_vencimiento=fv, actividad_flujo=af,
        )
    except Exception as e:
        return HTMLResponse(str(e), status_code=400)
...
def gastos_pagar(gid: int, cuenta_origen: str = Form("104"), permitir_sobregiro: str = Form(""),
                 db: Session = Depends(get_db), user=FinanzasAuth):
    # Pago atómico: pasivo real (4111/4212/4241…)/caja-banco + EGRESO flujo
    # por naturaleza + caja (bloquea período cerrado; 1011 bloquea sin saldo,
    # 1041 exige el flag permitir_sobregiro).
    from app.services import contabilidad as contab
    try:
        ...
        return _error_pago_400("/finanzas/gastos", str(e))
    return RedirectResponse("/finanzas/gastos", status_code=303)
...
def _fila_rentabilidad(db: Session, o, umbral: float, tarifa_media: float) -> dict:
    ...
    try:
        salidas = db.query(MovimientoKardex).filter(
            ...
            MovimientoKardex.tipo_movimiento.in_(
                ["SALIDA_CONSUMO_TALLER", "SALIDA_TALLER"])).all() or []
    except Exception:
        salidas = []
    if salidas:
        try:
    ...
    # Costo de M.O.: minutos por absorción (tareo → sam → SAM estándar,
    # None-safe). Sin tareo registrado usa el SAM de la ficha o el estándar
    # por tipo de prenda, para que el pedido terminado siempre absorba MOD.
    try:
        minutos, _fte_mod = svc.minutos_mod_pedido(db, o.id)
    except Exception:
        minutos, _fte_mod = 0.0, "sin_datos"
    try:
...
def rentabilidad(request: Request, umbral: float = Query(40.0), db: Session = Depends(get_db), user=FinanzasAuth):
    ...
    for o in orders:
        try:
            ...
            rows.append({"order": o, "precio": 0.0, "costo_telas": 0.0,
                         ...
                         "fuente_costo": "pendiente", "pendiente_costeo": True,
                         "pendiente_liquidacion": True})
    # KPIs de control de gestión (None/Decimal-safe: todo a float antes
    # de operar; sin divisiones por cero; un ER nulo no tumba la página).
    ...
    def _f(x) -> float:
        try:
            ...
            return 0.0
    cerrados=[r for r in rows if not r.get("pendiente_liquidacion") and r.get("margen_pct") is not None]
    try:
        margen_prom=round(sum(_f(r.get("margen_pct")) for r in cerrados)/len(cerrados),1) if cerrados else 0.0
    except Exception:
        margen_prom=0.0
    mes_ini=_date.today().replace(day=1)
    try:
        er_mes=svc.obtener_estado_resultados(db, desde=mes_ini) or {}
    except Exception as e:
        logger.error("rentabilidad: estado resultados falló (%s)", e)
        er_mes={}
    opex_mes=round(_f(er_mes.get("gastos_admin"))+_f(er_mes.get("gastos_ventas"))+_f(er_mes.get("gastos_financieros"))+_f(er_mes.get("gastos_operativos")),2)
    try:
        punto_equilibrio=round(opex_mes/(margen_prom/100),2) if margen_prom>0 else 0.0
    except Exception:
        punto_equilibrio=0.0
    try:
        sam_est=_f(db.query(func.coalesce(func.sum(Garment.sam_estimado),0)).scalar())
    except Exception:
        sam_est=0.0
    try:
        sam_real=_f(db.query(func.coalesce(func.sum(Garment.sam_real),0)).scalar())
    except Exception:
        sam_real=0.0
    eficiencia=round(sam_est/sam_real*100,1) if sam_real>0 else 0.0
    try:
        ebitda=round(_f(er_mes.get("utilidad_bruta"))-opex_mes,2)
    except Exception:
        ebitda=0.0
