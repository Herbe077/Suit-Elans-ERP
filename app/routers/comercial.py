...
def crear_persona(nombre: str = Form(...), apellidos: str = Form(...),
    ...
    db.commit()
    db.refresh(c)
    # Las ventas nacen en Ventas y Atención, con el cliente preseleccionado.
    back = f"/ventas/ordenes?cliente_id={c.id}"
...
@router.post("/clientes/{cid}/medidas")
async def cliente_guardar_medidas(cid: int, request: Request,
                                  db: Session = Depends(get_db), user=Ventas):
    """Registra una nueva versión de medidas sin salir del CRM."""
    ...
    if partes and not all(p.endswith(": Normal") for p in partes):
        postura = " · ".join(partes)
    elif partes:
        postura = "Normal"
    else:
        postura = (form.get("postura") or "").strip() or None
    m = Measurement(client_id=cid, sastre_id=user.id,
                    tipo_prenda=form.get("tipo_prenda", "saco"),
                    postura=postura, observaciones=form.get("observaciones"),
                    **data)
    m.version = db.query(Measurement).filter(
        Measurement.client_id == cid).count() + 1
    db.add(m)
    db.commit()
    return RedirectResponse(
        f"/comercial/clientes/detalle?tipo=persona&id={cid}", status_code=303)
