...
class RecetaBOM(Base):
    """Ficha técnica / receta de colección: BOM asignada a un producto comercial.

    Una receta lista los insumos directos (telas, avíos) por UNIDAD producida.
    Al finalizar una OP de colección (COMPLETADA/FINALIZADA) se descuenta
    cantidad_por_unidad × unidades del stock físico de cada insumo.
    """
...
class RecetaBOMLinea(Base):
    """Línea de BOM: insumo directo + cantidad por unidad producida."""

    __tablename__ = "recetas_bom_lineas"

    id: Mapped[int] = mapped_column(primary_key=True)
    receta_id: Mapped[int] = mapped_column(ForeignKey("recetas_bom.id"), index=True)
    producto_insumo_id: Mapped[int] = mapped_column(ForeignKey("productos_insumo.id"), index=True)
    cantidad_por_unidad: Mapped[float] = mapped_column(Float, default=0.0)
