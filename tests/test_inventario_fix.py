...
def test_movimiento_normaliza_signo_y_kardex_valida():
    ...
    prod_id = 999999
    r = c.post("/inventario/almacen/kardex", data={"producto_id": str(prod_id), "tipo_movimiento": "ENTRADA_COMPRA", "cantidad": "0"}, cookies=ck)
...
def test_api_low_stock_completo():
    c, ck = _client()
    r = c.post("/api/v1/auth/token", data={"username": "admin@suitelans.mx", "password": os.environ.get("TEST_ADMIN_PASSWORD", "test-admin-password")})
