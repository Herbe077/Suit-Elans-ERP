...
def test_concepto_desde_alta_clientes(client, auth_cookies):
    """Alta en Clientes con concepto → Ventas con cliente y concepto listos."""
    from app.core.database import SessionLocal
    from app.models.client import Client
    ...
    assert "cliente_id=" in r.headers["location"]
    assert "concepto=" in r.headers["location"]
    db = SessionLocal()
    c = db.query(Client).filter(Client.nro_doc == "66778899").first()
    pid = c.id
    db.close()
    r = client.get(f"/ventas/ordenes?cliente_id={pid}&concepto=Smoking%20boda",
                   cookies=auth_cookies)
    assert r.status_code == 200
    assert "Smoking boda" in r.text
    r = client.post("/ventas/ordenes",
                    data={"client_id": str(pid), "garment_tipo": "smoking",
                          "total": "2000", "concepto": "Smoking boda"},
                    cookies=auth_cookies)
    assert r.status_code in (200, 303)
    db = SessionLocal()
