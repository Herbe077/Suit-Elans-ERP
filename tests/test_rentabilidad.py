...
def test_entregada_liquida_sin_etiqueta(client, auth_cookies):
    ...
    # Saco entregado sin tareo absorbe MOD por SAM estándar (600 min):
    # margen = (1000 − 600×tarifa) / 1000 (ya no 100.0%).
    from app.services import finanzas as _fz
    _tarifa = float(_fz.tarifa_minuto_taller(db) or 0.35)
    db.close()
    _mo = round(600.0 * _tarifa, 2)
    _pct = round((1000.0 - _mo) / 1000.0 * 100, 1)
    t = client.get("/finanzas/rentabilidad", cookies=auth_cookies).text
    assert "SE-RENT-004" in t and f"{_pct:.1f}%" in t
