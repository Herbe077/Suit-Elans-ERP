...
def test_kardex_pt_ingreso_venta_muestra():
    ...
    r = c.post("/inventario/almacen/kardex",
               data={**base, "tipo_movimiento": "ENTRADA_PRODUCTO_TERMINADO", "cantidad": "5"}, cookies=ck)
    ...
    r = c.post("/inventario/almacen/kardex",
               data={**base, "tipo_movimiento": "ENTRADA_COMPRA", "cantidad": "1"}, cookies=ck)
...
def test_kardex_mp_acepta_ajuste_inventario():
    ...
    assert k.tipo_movimiento == "SALIDA_AJUSTE"  # legacy normalizado
...
def test_pt_muestra_costo_y_tabs_filtran():
    ...
    assert "ENTRADA_PRODUCTO_TERMINADO" in a and "SALIDA_VENTA" in a and "MUESTRA" in a
