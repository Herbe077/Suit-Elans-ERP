...
TEST_ADMIN_PASSWORD = os.environ.get("TEST_ADMIN_PASSWORD", "test-admin-password")
...
cargar_plan_operativo(s, desde_cero=False)
s.commit()
if not s.query(User).filter(User.email == "admin@suitelans.mx").first():
    s.add(User(email="admin@suitelans.mx", full_name="Administrador",
               hashed_password=security.hash_password(TEST_ADMIN_PASSWORD), role="admin",
               is_active=True))
    s.commit()
s.close()
...
def auth_cookies(client):
    r = client.post("/auth/login",
                    data={"username": "admin@suitelans.mx", "password": TEST_ADMIN_PASSWORD},
