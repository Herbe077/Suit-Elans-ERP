...
anticipo y aplicación en el comprobante final (7032)."""
...
def test_anticipo_y_final_bespoke(client, auth_cookies):
    ...
    assert mapa2["7032"] == (Decimal("0"), Decimal("2000"))
    db.close()
    # UI: desglose en el desplegable y monto editable
    t = client.get("/ventas/facturacion", cookies=auth_cookies).text
    assert "Anticipo pend. de facturar" in t
    assert "Monto a comprobantar" in t
