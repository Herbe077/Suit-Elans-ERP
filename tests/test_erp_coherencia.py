...
def test_cierre_y_depreciacion():
    ...
    f.crear_asiento(db, date.today(), "COH venta", "VENTA", 9101, [
        ...
        {"cuenta_id": f.get_cuenta_by_codigo(db, "40111").id, "debe": Decimal("0"), "haber": Decimal("18")},
    ])
    ...
    assert f.get_cuenta_by_codigo(db, "6811").id in dep_cuentas
    assert f.get_cuenta_by_codigo(db, "3911").id in dep_cuentas
