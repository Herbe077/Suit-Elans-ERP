...
def test_planilla_sin_igv_y_4111():
    ...
    g, a = f.registrar_gasto_operativo(
        ...
        centro_costo_id=dim_centro(db, "941"))
...
def test_planilla_destino_por_centro():
    ...
    cargar_plan_operativo(db)
    _limpia(db)
    for codigo, tipo, esperado in (("T-PL", "TALLER", "9211"),
...
def test_rxh_alquiler_natural_y_activo_fijo():
    ...
    cargar_plan_operativo(db)
    _limpia(db)
    from app.models.purchasing import Supplier as _Sup
    _sup_rxh = _Sup(nombre="Sastre RxH", ruc="10444444444")
    db.add(_sup_rxh)
    db.commit()
    g, a = f.registrar_gasto_operativo(
        db, fecha=date.today(), categoria="HONORARIOS_RXH", monto_base=800,
        monto_igv=50, numero_comprobante="RX-001",
        proveedor_id=_sup_rxh.id)
    mapa = _mapa(db, a.id)
    ...
    _sup_nat = Supplier(nombre="Arrendador Natural", ruc="10456789012")
    db.add(_sup_nat)
    db.commit()
    g2, a2 = f.registrar_gasto_operativo(
        db, fecha=date.today(), categoria="ALQUILER_NATURAL", monto_base=1200,
        numero_comprobante="ALQ-NAT-001", proveedor_id=_sup_nat.id)
    db.query(Supplier).filter(Supplier.id == _sup_nat.id).delete()
    db.commit()
    ...
    assert mapa3["3351"] == (Decimal("1000"), Decimal("0"))
    ...
    db.query(_Sup).filter(_Sup.id == _sup_rxh.id).delete()
    db.commit()
    _limpia(db)
    db.close()
...
def test_pago_planilla_usa_4111():
    ...
    g, _a = f.registrar_gasto_operativo(
        ...
        centro_costo_id=dim_centro(db, "941"))
    f.pagar_gasto(db, g.id, permitir_sobregiro=True)
    db.refresh(g)
