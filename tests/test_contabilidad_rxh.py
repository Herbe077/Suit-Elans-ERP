...
def test_gasto_manual_rxh_destino_9211():
    ...
    db = SessionLocal()
    from app.models.purchasing import Supplier as _SupM
    _sup_man = _SupM(nombre="Sastre Manual", ruc="10455555555")
    db.add(_sup_man)
    db.commit()
    g, a = f.registrar_gasto_operativo(
        ...
        retencion=80, proveedor_id=_sup_man.id)
    ...
    from app.models.purchasing import Supplier as _SupM2
    db.query(_SupM2).filter(_SupM2.id == _sup_man.id).delete()
    db.commit()
    db.close()
