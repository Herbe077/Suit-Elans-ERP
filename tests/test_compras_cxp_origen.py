...
def test_reparar_backfill_asiento_billed_legado():
    ...
    assert mapa["6021"] == (Decimal("737.29"), Decimal("0"))
...
def test_gasto_activo_fijo_origen_e_inversion():
    ...
    g, _a = f.registrar_gasto_operativo(
        ...
        proveedor_id=sup.id)
    # CxP inmediata al registrar (sincronizar solo backfill: no duplica)
...
def test_origenes_legacy_se_normalizan():
    ...
    g, _a = f.registrar_gasto_operativo(
        ...
        proveedor_id=sup.id)
    # Los valores legacy se normalizan antes de persistir bajo el nuevo
    # constraint de catálogo cerrado.
    assert f.normalizar_origen_cxp("GASTOS") == "GASTOS OPERATIVOS"
    assert f.normalizar_origen_cxp("GASTOS_OPERATIVOS") == "GASTOS OPERATIVOS"
    assert f.normalizar_origen_cxp("valor_desconocido") == "GASTOS OPERATIVOS"
    _limpia(db)
    db.close()
